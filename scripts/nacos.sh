...
if [ "$OS_TYPE" = "redhat" ]; then
    cd /etc/yum.repos.d/
    wget http://mirrors.aliyun.com/docker-ce/linux/centos/docker-ce.repo || curl -O http://mirrors.aliyun.com/docker-ce/linux/centos/docker-ce.repo
...
# Download and import MySQL schema
echo -e "\033[32m#################### Downloading official Nacos server zip... ####################\033[0m"
cd $TEMP_DIR
...
docker cp nacos/conf/mysql-schema.sql mysql:/mysql-schema.sql
