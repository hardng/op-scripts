...
# Features:
#   - Rsync Pull: Sync Jenkins files from remote server to local
#   - Local Packing: Compress synced files into .tar.gz
#   - S3 integration (Upload & Retention) - Ported from nacos_backup.sh
# ==============================================================================

# --- Default Configurations ---
# Source (Local Jenkins) Defaults
SOURCE_PATH=""
...
Core Options:
  --backup                      Perform a jenkins backup (Local Rsync -> Pack -> S3).
  -h, --help                    Display this help message.

Source Configuration:
  --source-path <path>          Local Jenkins Directory (e.g., /var/lib/jenkins)
...
Examples:
  $0 --backup --source-path /var/lib/jenkins --dir /tmp/jenkins_backup --s3 ...
EOF
    exit 0
}
...
# --- S3 Operations (Ported from Nacos Script) ---
setup_mcli_alias() {
    local mcli_cmd=$1
    if [ -n "$S3_ENDPOINT" ] && [ -n "$S3_ACCESS_KEY" ] && [ -n "$S3_SECRET_KEY" ]; then
        ...
        # Force Path-Style lookup for Aliyun Access Points
        local lookup_flag=""
...
cleanup_s3() {
    ...
    if [ "$S3_KEEP_COUNT" -gt 0 ]; then
        ...
        local files
        if [[ "$mcli_cmd" == *"docker"* ]]; then
            files=$(eval "$mcli_cmd ls --json \"$target\"" | grep "jenkins_backup_" | sed 's/.*"key":"\([^"]*\)".*/\1/' | sort -r)
        else
            files=$($mcli_cmd ls --json "$target" | grep "jenkins_backup_" | sed 's/.*"key":"\([^"]*\)".*/\1/' | sort -r)
        fi
...
cleanup_local() {
    ...
    if [ "$RETENTION_DAYS" -gt 0 ]; then
        ...
        find "$BACKUP_DIR" -maxdepth 1 -name "jenkins_backup_*.tar.gz" -mtime +$RETENTION_DAYS -exec rm -f {} \;
    fi
    ...
    if [ "$KEEP_COUNT" -gt 0 ]; then
        ...
        ls -t "$BACKUP_DIR"/jenkins_backup_*.tar.gz 2>/dev/null | tail -n +$((KEEP_COUNT + 1)) | xargs -r rm -f
    fi
}
...
do_backup() {
    ...
    if [ -z "$SOURCE_PATH" ]; then
        error "Missing required source argument: --source-path"
        usage
    fi
    ...
    local sync_dir="${BACKUP_DIR}/jenkins_sync"
    local filename="jenkins_backup_${timestamp}.tar.gz"
    local filepath="${BACKUP_DIR}/${filename}"
    
    # Ensure sync dir is empty to avoid stale files
    rm -rf "$sync_dir"
    mkdir -p "$sync_dir"

    # 3. Local Rsync
    log "Starting Filtered Copy from ${SOURCE_PATH} to ${sync_dir}..."
    
    # Rsync command with inclusions/exclusions (Local copy)
    rsync -aq \
        --include='jobs/' \
        --include='jobs/*/' \
        ...
        "${SOURCE_PATH}/" "$sync_dir/" || {
            error "Rsync filtered copy failed."
            exit 1
        }
    ...
    if [ -s "$filepath" ]; then
        ...
        if [ "$s3_success" = true ] && [ "$S3_CLEANUP_LOCAL" = true ]; then
            log "S3 upload successful, deleting local tarball: $filepath"
            rm -f "$filepath"
        fi
        
        # ALWAYS clean up the temporary sync directory to save space
        log "Cleaning up temporary staging directory: $sync_dir"
        rm -rf "$sync_dir"
...
# Sanitize S3_ALIAS: Must start with a letter for mcli
if [[ "$S3_ALIAS" =~ ^[0-9] ]]; then
    log "S3 Alias starts with a digit, prefixing with 's3-' for mcli compatibility."
